/**
 * Centralized indexing policy for TD Realty Ohio quality gates.
 *
 * Recovered from seo-autopilot/lib/indexing-policy.mjs, which was removed along
 * with the rest of the seo-autopilot subsystem in 766b704 while three scripts
 * still imported it (indexing-guard, check-index-policy,
 * generate-index-quality-report). It lives under scripts/lib/ now because those
 * scripts are its only consumers.
 *
 * DEFAULT_POLICY below is the policy actually in force: the consolidated site is
 * ten routes (see src/config/routes.js), all indexable and all in the sitemap,
 * so there are no route-specific rules left to express. 404.html carries its own
 * noindex and is excluded from the sitemap by the excludeNoindex rule. An
 * optional scripts/config/indexing-policy.json can override the default if
 * route-specific rules are ever needed again.
 */
